...
geometries3 = [o3d.geometry.TriangleMesh.create_coordinate_frame(0.5)]
...
geometries3.append(pcd_pick_grasp)
geometries3.append(pcd_pick_scene)

demo_cup = extract_coloured_points(geometries3, colour="red")
demo_cup.paint_uniform_color([0, 0, 1])  # green
...
#Inverse pick_pose
task_cup.translate(-pick_pose[4:])
task_cup.rotate(o3d.geometry.get_rotation_matrix_from_quaternion(pick_pose[:4]).T, center=[0, 0, 0])

#Apply place_pose
place_pose = torch.load(f'{root1}/step_1/target_poses/poses.pt').numpy()[0]
task_cup.rotate(o3d.geometry.get_rotation_matrix_from_quaternion(place_pose[:4]), center=[0, 0, 0])
task_cup.translate(place_pose[4:])
...
demo_cup.translate(-pick_pose[4:])
demo_cup.rotate(o3d.geometry.get_rotation_matrix_from_quaternion(pick_pose[:4]).T, center=[0, 0, 0])

demo_cup.rotate(o3d.geometry.get_rotation_matrix_from_quaternion(place_pose[:4]), center=[0, 0, 0])
demo_cup.translate(place_pose[4:])

demo_cup.rotate(R_green, center=[0, 0, 0])
demo_cup.translate(green_trans)





place_grasp_points = torch.load(f'{root2}/step_1/grasp_pcd/points.pt')
place_grasp_colors = torch.load(f'{root2}/step_1/grasp_pcd/colors.pt')
place_scene_points = torch.load(f'{root2}/step_1/scene_pcd/points.pt')
place_scene_colors = torch.load(f'{root2}/step_1/scene_pcd/colors.pt')
place_pose = torch.load(f'{root2}/step_1/target_poses/poses.pt').numpy()[0]
...
# geometries1.append(demo_cup)
geometries1.append(pcd_place_grasp)
